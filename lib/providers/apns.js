...
function ApnsProvider(pushSettings) {
  ...
  // TODO: Remove the certificate stuff completly
  ...
  if (typeof settings.token === 'undefined' || typeof settings.bundle === 'undefined'){
    var err = new Error('No JWT Token or bundle was defined for APNs');
...
ApnsProvider.prototype.pushNotification = function(notification, deviceToken) {
  ...
  self._ensurePushConnection(this._pushOptions);
  ...
  // the topic is nessecary to identify
  // the app which receives this notification
  note.topic = self._pushOptions.bundle;
