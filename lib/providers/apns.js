...
ApnsProvider.prototype._setupPushConnection = function(options) {
  ...
  connection.on("completed", function() {
    debug("Notification Queue Empty!");
  });

  connection.on("connected", function(openSockets) {
    debug("Connected to apn server ", openSockets);
  });

  connection.on('disconnected', function(openSockets) {
    debug("Disconnected from apn server ", openSockets);
    var err = new Error('apn disconnected');
    self.emit('error', err);
  });

  connection.on('timeout', function() {
    debug("Idle connection to apn timed out");
  ...
  connection.on('cacheTooSmall', function(sizeDifference) {
    debug("cacheTooSmall ", sizeDifference);
  });

  connection.on('transmissionError', function(code, notification, recipient) {
    var err = new Error('Cannot send apn: ' + code);
    err.code = code;
