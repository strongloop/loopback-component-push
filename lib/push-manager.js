 ...
 * @returns {Provider|null} A provider from PushManager.providers (GcmCcsProvider, ApnsProvider, WnsProvider)
...
PushManager.prototype.configureProvider = function (deviceType, pushSettings) {
  ...
  provider.on('devicesGone', function(deviceTokens) {
    this.emit('devicesGone', deviceTokens, deviceType);
  ...
  provider.on('error', function(err, deviceType) {
    this.emit('error', err, deviceType);
  }.bind(this));

  provider.on('transmitted', function(notif, result) {
    this.emit('transmitted', notif, result);
  }.bind(this));

  provider.on('transmissionError', function(err, notification, recipient) {
    this.emit('transmissionError', err, notification, recipient);
  }.bind(this));

  provider.on('gcm-ccs-disconnected', function() {
    this.emit('gcm-ccs-disconnected');
  }.bind(this));
