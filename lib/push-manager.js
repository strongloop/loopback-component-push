...
PushManager.prototype.configureProvider = function (deviceType, pushSettings) {
    var self = this;
    if (deviceType === 'ios') {
        var feedback = new (require('events').EventEmitter)();

        var push = new providers.ApnsProvider(pushSettings);
        push.on('device gone', function(deviceToken) {
          self.Device.destroyAll({
            deviceType: 'ios',
            deviceToken: deviceToken
          });
          feedback.emit('feedback', [deviceToken]);
        });
            ...
            push: {
                pushNotification: function (notification, deviceToken) {
                    if (notification instanceof self.Notification) {
                      ...
                      push._connection.pushNotification(notification, deviceToken);
                    }
                }
            }
        };
    } else if (deviceType === 'android') {
        var sender = new providers.GcmProvider(pushSettings);
        return {
            push: {
                pushNotification: function (notification, deviceToken) {
                  sender.pushNotification(notification, deviceToken);
                }
            }
        };
    } else {
