...
PushManager.prototype.notify = function(installation, notification, cb) {
 ...
 /**
 * Push a notification to installations for given devices tokens, device type and app. 
 * 
...
PushManager.prototype.notifyMany = function(appId, deviceType, deviceTokens, notification, cb) {
    ...
    if(!(typeof notification === 'object' && notification)) {
        logger.error('notification must be an object', notification);
        return cb(new Error('notification must be an object'));
    }

    if(!(typeof deviceTokens === 'object' && deviceTokens && deviceTokens.length > 0)) {
        logger.error('deviceTokens must be an array', deviceTokens);
    ...
    this.configureApplication(
        ...
        function(err, provider) {
            if (err) {
                logger.error('Error configuring Application', err);
                return cb(err);
            }
