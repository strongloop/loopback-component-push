...
/**
 * Initialize the dependent models lazily to avoid race condition
 * as the push data source can be created before installation/notification
 * models are defined.
 *
 * @private
 */
PushManager.prototype._init = function() {
  if(!this.Installation) {
    var settings = this.settings;
    this.Installation = loopback.getModel(settings.installation)
      || loopback.getModelByType(Installation);
    this.Notification = loopback.getModel(settings.notification)
      || loopback.getModelByType(Notification);
    this.Application = loopback.getModel(settings.application)
