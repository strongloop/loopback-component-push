...
function PushManager(settings) {
    ...
    this.stdTTL = this.settings.stdTTL || 0;
    this.checkPeriod = this.settings.checkPeriod || 0;
    this.applicationsCache = new NodeCache({ stdTTL: this.stdTTL, checkperiod: this.checkPeriod });
}
...
PushManager.prototype.configureApplication = function (appId, deviceType, cb) {
  ...
  self.Application.findById(appId, function (err, application) {
    ...
    if (!cacheApp) {
        self.applicationsCache.set(appId, {});
    }
