...
function PushManager(settings) {
    ...
    this.DeviceRegistration = settings.DeviceRegistration || require('../models/device-registration');
...
PushManager.prototype.configureProvider = function (deviceType, pushSettings) {
    ...
    if (deviceType === 'ios') {
        ...
        if (feedbackOptions) {
            ...
            feedback.on("feedback", function (devices) {
                devices.forEach(function (device) {
                    self.DeviceRegistration.deleteAll({where: {deviceType: 'ios', deviceToken: device}});
                });
            });
        }
        ...
        return {feedback: feedback, push: push};
        ...
        return {
            push: {
                pushNotification: function (notification, deviceToken) {
                    ...
                    var message = new new gcm.Message({
...
PushManager.prototype.pushNotification = function (deviceToken, notification) {
    ...
    this.DeviceRegistration.findOne({where: {deviceToken: deviceToken}}, function(err, reg) {
        if(reg) {
...
PushManager.prototype.pushNotificationByApp = function (deviceType, appId, appVersion, notification, cb) {
    ...
    self.DeviceRegistration.findByApp(deviceType, appId, appVersion, function (err, regs) {
        if (!err) {
...
PushManager.prototype.pushNotificationByUser = function (deviceType, userId, notification, cb) {
    ...
    self.DeviceRegistration.findByUser(deviceType, userId, function (err, regs) {
        if (!err) {
            regs.forEach(function (r) {
                self.pushNotification( r.deviceToken, notification);
            });
